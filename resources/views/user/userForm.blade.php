                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card card-default">
                            ...
                            <div class="card-body">
                                @if(isset($data))
                                    <form action="{{ route('actionEditUser', $data->id) }}" method="post">
                                    @csrf
                                    @method('PUT')
                                @else
                                    <form action="{{ route('actionAddUser') }}" method="post">
                                        @csrf
                                @endif
                                    <div class="form-group">
                                        <label>Nama</label>
                                        <input type="text" name="nama" class="form-control" placeholder="Nama" required="" value="{{ isset($data) ? $data->name : '' }}">
                                    </div>
                                    <div class="form-group">
                                        <label>Email</label>
                                        <input type="email" name="email" class="form-control" placeholder="Email" value="{{ isset($data) ? $data->email : '' }}">
                                    </div>
                                    <div class="form-group">
                                        <label>Role</label>
                                        <select class="custom-select" name="role">
                                            ...
                                            @foreach ($role as $item)
                                                @php
                                                    if((isset($data) ? $data->role_id : '') == $item->id){
                                                        $check = 'selected';
                                                    }else{
                                                        $check = '';
                                                    }
                                                @endphp
                                                <option value="{{$item->id}}" {{ $check }}>{{$item->nama_role}}</option>
                                            @endforeach
                                        </select>
                                    </div>
